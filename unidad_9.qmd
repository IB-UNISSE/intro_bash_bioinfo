...
``` bash
apptainer build --sandbox mi_contenedor ubuntu_latest.sif
```

`NBODY` **es el nombre del directorio que se va a crear**
...
``` bash
apptainer shell --writable --fakeroot --pwd / mi_contenedor
```
...
``` bash
sudo apptainer build mrbayes_mpi.sif mi_contenedor
```
...
%environment
    ...
    # Evita errores con caracteres especiales y asegura compatibilidad.
    export LANG=C.UTF-8
    export LC_ALL=C.UTF-8

    # Indica dónde buscar bibliotecas compartidas al ejecutar programas.
    # Es importante cuando se instalan librerías manualmente en /usr/local/lib
...
# Aquí es donde realmente se instala y configura el software.
...
%post
    ...
    # --------------------------------------------------------
    # Se instalan herramientas necesarias para compilar software
    # y librerías requeridas por RevBayes.

    apt install -y \
        build-essential \   # compiladores (gcc, g++)
        cmake \             # sistema de construcción
        git \               # descarga código fuente
        libopenmpi-dev \    # librerías de MPI (paralelización)
        openmpi-bin \       # ejecutables MPI (mpirun)
        libboost-all-dev    # librerías Boost necesarias

    # --------------------------------------------------------
    # Descargar código fuente
    # --------------------------------------------------------
    # Se descarga el repositorio de RevBayes desde GitHub.
    # Se usa la rama 'development', que es la recomendada para compilación.
    ...
    # --------------------------------------------------------
    # Se entra al directorio de compilación y se ejecuta el script build.sh
    # con soporte para MPI activado.
    ...
    # --------------------------------------------------------
    # Se copia el ejecutable generado a una ruta estándar del sistema
    # para que pueda ejecutarse desde cualquier lugar.
...
%runscript
    # Mensaje informativo al iniciar el contenedor
    echo "RevBayes con MPI listo para usarse"

    # Ejecuta cualquier comando que el usuario pase al contenedor
    # "$@" representa todos los argumentos recibidos
    exec "$@"
```
...
👉 [Docker Hub]{.underline} ([https://hub.docker.com](https://hub.docker.com/))
...
``` bash
apptainer pull docker://cyverseuk/mrbayes
```
